...
public:
    ...
    void changeCursor(int line, int column, int count);
    
    int returnLastCharLocation(int line);

    void insertCharacter(char x, int column, int line);
    std::string deleteCharacter(int column, int line);

    void newVectorElement(int line, int column);
    void removeVectorElement(int line, int column);
    void returnElementofVector(int lineNumber,std::string thetext);
    std::string changeElementofVector(int lineNumber, std::string thetext);

    std::string returnTextofLine(int linenumber);
    std::string returnBeginningtext(int column, std::string thetext);
    std::string returnEndtext(int column,int line);
 /*   void moveRight();
    void moveLeft();
    void moveUp();
    void moveDown();
...
private:
    ...
    int current_cursorLine;
    int current_cursorColumn;
    int current_lineCount;
    std::string current_line;
    
    std::string text;
    std::vector<std::string> theline{1,""};
